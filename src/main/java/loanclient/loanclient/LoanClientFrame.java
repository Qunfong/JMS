...
public class LoanClientFrame extends JFrame {
	...
	private DefaultListModel<RequestReply<LoanRequest,LoanReplyBroker>> listModel = new DefaultListModel<RequestReply<LoanRequest,LoanReplyBroker>>();
	private JList<RequestReply<LoanRequest,LoanReplyBroker>> requestReplyList;
	...
	public LoanClientFrame() {
		...
		btnQueue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				listModel.addElement( new RequestReply<LoanRequest,LoanReplyBroker>(request, null));
				// to do:  send the JMS with request to Loan Broker
				try {
					loanBrokerAppGateway = new LoanBrokerAppGateway(correlations);
		...
		requestReplyList = new JList<RequestReply<LoanRequest,LoanReplyBroker>>(listModel);
	 ...
	 * @return
	 */
   private RequestReply<LoanRequest,LoanReplyBroker> getRequestReply(LoanRequest request){
     
     for (int i = 0; i < listModel.getSize(); i++){
    	 RequestReply<LoanRequest,LoanReplyBroker> rr =listModel.get(i);
	...
	public void add(LoanRequest loanRequest, LoanReply loanReply) {
		RequestReply rr = getRequestReply(loanRequest);
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					new LoanReplyBroker(frame).onLoanReplyArrived();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
