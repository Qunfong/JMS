...
public class JMSBankFrame extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					new LoanBrokerAppGateway(frame).onBankIntererstRequestArrived();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		btnSendReply.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (rr!= null && reply != null){
					...
					LoanBrokerAppGateway loanBrokerAppGateway = new LoanBrokerAppGateway(frame);
					try {
						loanBrokerAppGateway.sendReplyToBroker(reply, correlations.get(rr.getRequest()));
	...
	public void add(BankInterestRequest bankInterestRequest, String correlation){
		correlations.put(bankInterestRequest, correlation);
		listModel.addElement( new RequestReply<BankInterestRequest, BankInterestReply>(bankInterestRequest, null));
	}
